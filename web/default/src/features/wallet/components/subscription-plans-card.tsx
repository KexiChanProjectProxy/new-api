								...
								{allSubscriptions.map((sub) => {
									...
									return (
										...
										>
											...
											{(() => {
												const windowStates =
													subscription?.quota_window_states || [];
												const isWindowed = windowStates.length > 0;

												if (isWindowed && isActive) {
													return (
														<div className="mt-2 space-y-2">
															...
															{windowStates.map((ws) => {
																const wsLimit = Number(ws.quota || 0);
																const wsUsed = Number(ws.used || 0);
																const wsRemaining = Math.max(
																	0,
																	wsLimit - wsUsed,
																);
																const wsPercent =
																	wsLimit > 0
																		? Math.round((wsUsed / wsLimit) * 100)
																		: 0;
																const countdown = formatWindowResetCountdown({
				...
				{plans.length > 0 ? (
					<div className="grid grid-cols-1 gap-3 2xl:grid-cols-2 2xl:gap-4">
						{plans.map((p, index) => {
							...
							const quotaWindows = plan.quota_windows || [];
							const hasWindows = quotaWindows.length > 0;
							...
							const benefits = [
								...
								...(!hasWindows
									...
									: quotaWindows.map(
											(w) =>
												`${t("Quota")} · ${formatWindowLabel(w)}: ${formatQuota(w.quota)}`,
										)),
