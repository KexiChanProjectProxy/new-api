...
export function SubscriptionsMutateDrawer({
	...
	const windowDurationUnitOpts = getWindowDurationUnitOptions(t);
					...
					>
						...
						<SideDrawerSection>
							...
							<p className="text-muted-foreground text-xs">
								{fields.length > 0
									? t(
											"Windowed quotas replace the legacy total quota. Each window tracks its own independent quota and reset cycle.",
										)
									: t(
											"Define time-bucketed quota windows. Leave empty to use the legacy total quota above.",
										)}
							</p>
							...
							{fields.map((field, index) => (
								<div
									key={field.id}
									className="border-border/40 flex flex-col gap-2 rounded-md border p-3"
								>
									...
									<div className="grid grid-cols-2 gap-2">
										<FormField
											control={form.control}
											name={`quota_windows.${index}.duration_value`}
											render={({ field: f }) => (
												<FormItem>
										...
										<FormField
											control={form.control}
											name={`quota_windows.${index}.duration_unit`}
											render={({ field: f }) => (
												<FormItem>
													<FormLabel className="text-xs">{t("Unit")}</FormLabel>
													<Select
														items={windowDurationUnitOpts.map((o) => ({
															value: o.value,
															label: o.label,
														}))}
									...
									<FormField
										control={form.control}
										name={`quota_windows.${index}.display_quota`}
										render={({ field: f }) => (
											<FormItem>
												<FormLabel className="text-xs">
													{t("Display Quota")} (USD)
